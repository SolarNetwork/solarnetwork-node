 ...
 * @version $Revision$
 */
public class JdbcWeatherDatumDao
extends AbstractJdbcDatumDao<WeatherDatum> {
	...
	private static final String DEFAULT_SQL_INSERT = "INSERT INTO " 
		+SCHEMA_NAME +'.'+TABLE_WEATHER_DATUM
		+" (source_id, info_date, sky_cond, temperature, humidity," 
		+" bar_pressure, bar_delta, visibility, uv_index, dew_point,"
		+" error_msg) VALUES"
		+" (?,?,?,?,?,?,?,?,?,?,?)";
	...
	private static final String DEFAULT_SQL_INSERT_UPLOAD = "INSERT INTO " 
		+SCHEMA_NAME +'.'+TABLE_WEATHER_DATUM_UPLOAD
		+" (datum_id, destination, track_id) VALUES (?,?,?)";
	
	private static final String DEFAULT_SQL_DELETE_OLD = "DELETE FROM "
		+SCHEMA_NAME +'.' +TABLE_WEATHER_DATUM +" where id <= "
		+"(select MAX(cd.id) from " +SCHEMA_NAME +'.' +TABLE_WEATHER_DATUM 
		+" cd inner join " +SCHEMA_NAME +'.' +TABLE_WEATHER_DATUM_UPLOAD 
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public DatumUpload storeDatumUpload(WeatherDatum datum,
	...
	@Override
	protected void setStoreStatementValues(WeatherDatum datum, PreparedStatement ps) 
	throws SQLException {
		int col = 1;
