...
public class WMBusDatumDataSourceConfigCsvParserTests {
	...
	public void parse_deviceDetails_explicitKeys() throws IOException {
		...
		try (Reader in = new InputStreamReader(
				getClass().getResourceAsStream("test-wmbus-config-01.csv"), ByteUtils.UTF8);
	...
	public void parse_deviceDetails_implicitKeys() throws IOException {
		...
		// WHEN
		try (Reader in = new InputStreamReader(
				getClass().getResourceAsStream("test-wmbus-config-02.csv"), ByteUtils.UTF8);
				ICsvListReader csv = new CsvListReader(in, CsvPreference.STANDARD_PREFERENCE)) {
			parser.parse(csv);
		}
	...
	public void parse_deviceDetails_mixedKeys() throws IOException {
		...
		// WHEN
		try (Reader in = new InputStreamReader(
				getClass().getResourceAsStream("test-wmbus-config-03.csv"), ByteUtils.UTF8);
				ICsvListReader csv = new CsvListReader(in, CsvPreference.STANDARD_PREFERENCE)) {
			parser.parse(csv);
		}
	...
	public void parse_sample01() throws IOException {
		...
		// WHEN
		try (Reader in = new InputStreamReader(
				getClass().getResourceAsStream("test-wmbus-config-sample-01.csv"), ByteUtils.UTF8);
				ICsvListReader csv = new CsvListReader(in, CsvPreference.STANDARD_PREFERENCE)) {
			parser.parse(csv);
		}
