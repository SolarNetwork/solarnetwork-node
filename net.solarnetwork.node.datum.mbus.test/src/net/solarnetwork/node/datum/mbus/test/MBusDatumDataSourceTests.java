...
public class MBusDatumDataSourceTests {
	...
	private MBusDatumDataSource dataSource;
	private MBusNetwork mbusNetwork;
	private MBusConnection mbusConnection;

	@Before
	public void setup() {
		mbusNetwork = EasyMock.createMock(MBusNetwork.class);
	...
	private void replayAll() {
		EasyMock.replay(mbusNetwork, mbusConnection);
	}

	@After
	public void teardown() {
		EasyMock.verify(mbusNetwork, mbusConnection);
	}
	...
	public void readDatumWithInstantaneousValues() throws IOException {
		...
		// WHEN
		expect(mbusNetwork.read(1)).andAnswer(new IAnswer<MBusData>() {

			@Override
			public MBusData answer() throws Throwable {
				return mbusConnection.read();
			}
		});
