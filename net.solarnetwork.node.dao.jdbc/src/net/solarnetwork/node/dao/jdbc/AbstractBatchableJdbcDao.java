 ...
 * 
 * @param <T> the type of domain object this DAO supports
 * @author matt
 * @version 1.0
 */
public abstract class AbstractBatchableJdbcDao<T> extends SimpleJdbcDaoSupport implements BatchableDao<T> {
	 ...
	 * 
	 * @param options the requested batch options
	 ...
	 * 
	 * @param options the requested batch options
	 * @param resultSet the current ResultSet, positioned on the next row
	 * @param rowCount the current count of rows processed (1-based)
	 * @return the entity
	 * @throws SQLException if any SQL error occurs
	 */
	protected abstract T getBatchRowEntity(BatchOptions options, ResultSet resultSet, int rowCount) throws SQLException;
	
	/**
	 * Update the current row in a ResulSet for batch processing.
	 * 
	 * <p>The {@link ResultSet#updateRow()} method should <strong>not</strong> be called
	...
	private BatchResult batchProcessInternal(final BatchCallback<T> callback, final BatchOptions options) {
		...
		getJdbcTemplate().execute(new ConnectionCallback<Object>() {
			@Override
			public net.solarnetwork.node.dao.BatchableDao.BatchResult doInConnection(
					Connection con) throws SQLException, DataAccessException {
