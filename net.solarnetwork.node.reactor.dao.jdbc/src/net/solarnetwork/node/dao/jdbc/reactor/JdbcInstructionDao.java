...
implements net.solarnetwork.node.reactor.InstructionDao {
	...
	/** The classpath Resource for the SQL template for inserting an Instruction parameter. */
	public static final String RESOURCE_SQL_INSERT_INSTRUCTION_PARAM = "insert-param";
	
	/** The classpath Resource for the SQL template for inserting an InstructionStatus. */
	public static final String RESOURCE_SQL_INSERT_INSTRUCTION_STATUS = "insert-status";
	
	/** The classpath Resource for the SQL template for updating an InstructionStatus. */
	public static final String RESOURCE_SQL_UPDATE_INSTRUCTION_STATUS = "update-status";

	/** The classpath Resource for the SQL template for selecting Instruction by unique keys. */
	public static final String RESOURCE_SQL_SELECT_INSTRUCTION_FOR_KEYS = "select-for-keys";
	
	/** The classpath Resource for the SQL template for selecting Instruction by ID. */
	public static final String RESOURCE_SQL_SELECT_INSTRUCTION_FOR_ID = "select-for-id";
	
	/** The classpath Resource for the SQL template for selecting Instruction by state. */
	...
	/** The classpath Resource for the SQL template for deleting old Instruction rows. */
	...
	public Instruction getInstruction(Long instructionId) {
		return getJdbcTemplate().query(
				getSqlResource(RESOURCE_SQL_SELECT_INSTRUCTION_FOR_ID),
	...
	public Long storeInstruction(final Instruction instruction) {
		...
		final Long pk = storeDomainObject(instruction, 
				getSqlResource(RESOURCE_SQL_INSERT_INSTRUCTION));
		...
		getJdbcTemplate().execute(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con)
				...
				PreparedStatement ps = con.prepareStatement(
						getSqlResource(RESOURCE_SQL_INSERT_INSTRUCTION_PARAM));
				return ps;
			}
		...
		getJdbcTemplate().update(getSqlResource(RESOURCE_SQL_INSERT_INSTRUCTION_STATUS),
				pk, new java.sql.Timestamp(statusDate.getTime()), 
				InstructionState.Received.toString());
		return pk;
	}
	...
	public Instruction getInstruction(String instructionId, String instructorId) {
		return getJdbcTemplate().query(
				getSqlResource(RESOURCE_SQL_SELECT_INSTRUCTION_FOR_KEYS), 
				new Object[] {instructionId, instructorId}, 
				new ResultSetExtractor<Instruction>() {
					@Override
					public Instruction extractData(ResultSet rs)
							throws SQLException, DataAccessException {
	...
	private List<Instruction> extractInstructions(ResultSet rs) throws SQLException {
		...
		while ( rs.next() ) {
			...
			if ( bi == null || bi.getId().longValue() != currId ) {
				InstructionStatus status = new BasicInstructionStatus(
						currId, 
						InstructionState.valueOf(rs.getString(6)), 
						rs.getTimestamp(7), 
						(rs.getString(8) == null ? null : InstructionState.valueOf(rs.getString(8))));
				bi = new BasicInstruction(
						currId,
						rs.getString(2),
						rs.getTimestamp(3),
						rs.getString(4),
	...
	public void storeInstructionStatus(Long instructionId, InstructionStatus status) {
		getJdbcTemplate().update(
				...
				(status.getAcknowledgedInstructionState() == null 
					? null : status.getAcknowledgedInstructionState().toString()), 
				instructionId);
	...
	public List<Instruction> findInstructionsForState(InstructionState state) {
		return getJdbcTemplate().query(
				getSqlResource(RESOURCE_SQL_SELECT_INSTRUCTION_FOR_STATE), 
	...
	public int deleteHandledInstructionsOlderThan(final int hours) {
		return getJdbcTemplate().update(new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(Connection con)
				...
				log.debug("Preparing SQL to delete old instructions [{}] with hours [{}]",
						sql, hours);
