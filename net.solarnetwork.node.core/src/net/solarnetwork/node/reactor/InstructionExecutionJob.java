 ...
 * 
 * <p>This job uses the {@link InstructionDao} to query for all {@link Instruction}
 ...
 * that can execute the instruction.</p>
 * 
 * <p>This job will pass {@link Instruction#getTopic()} to every configured 
 ...
 * (because it does not support the given Instruction) this job will move
 * on to the next configured handler and try it.</p>
 * 
 * <p>The first handler to return an {@link InstructionState} other than 
 * {@link InstructionState#Received} will cause this job to persist the 
 * updated Instruction state and not pass the instruction to any other
 * handlers.</p>
 * 
 ...
 *   <dd>The {@link InstructionDao} to manage {@link Instruction} entities with.</dd>
 *   
 *   <dt>executionReceivedHourLimit</dt>
 *   <dd>The minimum amount of time to wait before forcing instructions into the
 *   {@link InstructionState#Declined} state. This prevents instructions not handled
 *   by any handler from sticking around on the node indefinitely. Defaults to 
 *   {@link #DEFAULT_EXECUTION_RECEIVED_HOUR_LIMIT}.</dd>
 *   
 *   <dt>handlers</dt>u
 ...
 * @version $Revision$
 */
public class InstructionExecutionJob extends AbstractJob implements StatefulJob {
	...
	protected void executeInternal(JobExecutionContext jobContext)
			throws Exception {
		List<Instruction> instructions = instructionDao.findInstructionsForState(
		...
		for ( Instruction instruction : instructions ) {
			...
			if ( !handled && instruction.getInstructionDate() != null ) {
				...
				if ( diffMs > timeLimitMs ) {
					...
					instructionDao.storeInstructionStatus(instruction.getId(), 
							instruction.getStatus().newCopyWithState(
