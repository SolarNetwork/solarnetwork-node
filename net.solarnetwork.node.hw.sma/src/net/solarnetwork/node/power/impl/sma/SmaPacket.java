 ...
 * 
 * <p>This is the basic unit of communication for requests to and responses from the
 * SMA controller. The data is encoded into/decoded from a byte array (the
 * {@code packet} field) from/to other fields on this class.</p>
 * 
 * <p>The packet structure consists of a fixed-length header, a variable-length
 * {@code userData} section, and a fixed-length footer. The header contains 
 * information like the length of the user data section, the source and destination
 * addresses, and the command type. The user data section contains any data
 * required to pass with the command, or sent back from the controller. The footer
 * contains a checksum value calculated from parts of the header and the complete
 * user data section.</p>
 * 
 * <p>The special {@code userDataFields} Map will contain data decoded from the
 * user data of response packets, such as the value returned by GetData commands.</p>
 * 
 * <p>The SMA controller uses little endian encoding in all communication.</p>
 * 
 * <pre>  # 2 optional sync bytes
...
public final class SmaPacket {
	 ...
	 * 
	 * <p>This constructor can be used for any type of packet, but generally
	 * will be used to construct a request packet to send to the controller.</p>
	 * 
	 * @param src the source address (usually 0)
	 * @param dest the destination address (use 0 for a RequestGroup control type)
	 * @param packetCounter the packet counter (usually 0)
	 * @param control the request type (usually RequestSingle or RequestGroup)
	 * @param command the command to issue
	 * @param userData the data to send with the command
	 */
	public SmaPacket(int src, int dest, int packetCounter, SmaControl control, 
			SmaCommand command, byte[] userData) {
		this.srcAddress = src;
	 ...
	 * 
	 * <p>This constructor will decode a raw packet byte array into the
	 * individual fields on this class.</p>
	 * 
	 * @param packet the raw packet data to decode
	 */
	...
	/**
	 * Return <em>true</em> if this packet is valid according to the
	 * packet's CRC check.
	 * 
	 * @return boolean
	 */
	public boolean isValid() {
		return crc != 0 && computedCrc == crc 
			&& command != null && command != SmaCommand.Unknown;
	}
	
	@Override
	public String toString() {
		return "SmaPacket{command=" +command
	 ...
	 * associated values.
	 * 
	 * <p>After calling this method, use {@link #getUserDataField(SmaUserDataField)}
	 * to obtain values for specific fields.</p>
	 */
	public void decodeUserDataFields() {
		Map<SmaUserDataField, Object> results 
			= new EnumMap<SmaUserDataField, Object>(SmaUserDataField.class);
		switch ( this.command ) {
			case NetStart:
	...
	private void decodeGetDataUserDataFields(Map<SmaUserDataField, Object> results) {
		...
		results.put(SmaUserDataField.SecondsSince, 
				(0xFF & userData[5]) | ((0xFF & userData[6]) << 8)
				| ((0xFF & userData[7]) << 16) | ((0xFF & userData[8]) << 24));
		results.put(SmaUserDataField.TimeBasis, 
				(0xFF & userData[9]) | ((0xFF & userData[10]) << 8)
		...
		switch ( type1 ) {
			...
			case Analog:
				// # 2 byte integer
				results.put(SmaUserDataField.Value, 
						(0xFF & userData[13]) | ((0xFF & userData[14]) << 8));
				break;
	...
	private SmaChannelType decodeGetSetUserDataFieldsHeader(Map<SmaUserDataField, Object> results) {
		...
		results.put(SmaUserDataField.DataSets, 
				(0xFF & userData[3]) | ((0xFF & userData[4]) << 8));
		results.put(SmaUserDataField.SecondsSince, 
				(0xFF & userData[5]) | ((0xFF & userData[6]) << 8)
				| ((0xFF & userData[7]) << 16) | ((0xFF & userData[8]) << 24));
		results.put(SmaUserDataField.TimeBasis, 
	...
	private void decodeNetUserDataFields(Map<SmaUserDataField, Object> results) {
		...
		long serial = (0xFF & userData[0]) | ((0xFF & userData[1]) << 8)
			| ((0xFF & userData[2]) << 16) | ((0xFF & userData[3]) << 24);
