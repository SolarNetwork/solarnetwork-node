/* ==================================================================
 * ModbusControlCsvConfigurer.java - 20/09/2022 1:33:02 pm
 *
...
public class ModbusControlCsvConfigurer extends BasicIdentifiable
	...
	private static final Logger log = LoggerFactory.getLogger(ModbusControlCsvConfigurer.class);

	private final SettingsService settingsService;
	private final OptionalService<IdentityService> identityService;

	private String settingProviderId = ModbusControl.SETTING_UID;

	private @Nullable Throwable lastImportException;
	private @Nullable List<String> lastImportMessages;
	 ...
	 */
	public ModbusControlCsvConfigurer(SettingsService settingsService,
	...
	public String getSettingUid() {
		return "net.solarnetwork.node.control.modbus.csv";
	}

	@Override
	public String getDisplayName() {
		return "Modbus Control CSV Configurer";
	}
	...
	public List<SettingSpecifier> getSettingSpecifiers() {
		...
		results.add(new BasicFileSettingSpecifier(RESOURCE_KEY_CSV_FILE, null,
				new LinkedHashSet<>(List.of(".csv", ".csv.gz", "text/csv", "text/csv+gzip")), true));
	...
	public synchronized @Nullable SettingsUpdates applySettingResources(String settingKey,
		...
		List<ModbusControlConfig> configs = new ArrayList<>(8);
		try {
			for ( Resource r : resources ) {
				try {
					this.lastImportException = null;
					this.lastImportMessages = null;
					List<ModbusControlConfig> rConfigs = parseModbusConfigs(r);
					if ( rConfigs != null ) {
	...
	private List<ModbusControlConfig> parseModbusConfigs(Resource resource) throws IOException {
		List<ModbusControlConfig> configs = new ArrayList<>(8);
		lastImportMessages = new ArrayList<>(8);
		ModbusControlConfigCsvParser parser = new ModbusControlConfigCsvParser(configs,
	...
	private @Nullable SettingsUpdates toSettingsUpdates(@Nullable List<ModbusControlConfig> configs) {
		...
		for ( ModbusControlConfig config : configs ) {
		...
		return new SettingsCommand(settings, List.of(Pattern.compile(".*")));
	}
