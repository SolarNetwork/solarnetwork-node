/* ==================================================================
 * ModbusControlConfigCsvWriter.java - 20/09/2022 2:30:33 pm
 *
...
public class ModbusControlConfigCsvWriter {
	 ...
	 */
	public ModbusControlConfigCsvWriter(CsvWriter writer) throws UncheckedIOException {
		super();
		this.writer = ObjectUtils.requireNonNullArgument(writer, "writer");
		rowLen = ModbusControlCsvColumn.values().length;
		String[] row = new String[rowLen];
		for ( ModbusControlCsvColumn col : ModbusControlCsvColumn.values() ) {
	...
	public void generateCsv(String factoryId, String instanceId, List<Setting> settings)
		...
		ModbusControlConfig config = new ModbusControlConfig();
		...
		row[ModbusControlCsvColumn.UNIT_ID
				.getCode()] = (config.getUnitId() != null ? config.getUnitId().toString() : null);
		row[ModbusControlCsvColumn.SAMPLE_CACHE.getCode()] = (config.getSampleCacheMs() != null
				? config.getSampleCacheMs().toString()
				: null);
		row[ModbusControlCsvColumn.WORD_ORDER.getCode()] = modbusWordOrderValue(config.getWordOrder());

		for ( ModbusWritePropertyConfig propConfig : config.getPropertyConfigs() ) {
			row[ModbusControlCsvColumn.CONTROL_ID.getCode()] = propConfig.getControlId();
			row[ModbusControlCsvColumn.PROP_TYPE.getCode()] = controlPropertyTypeValue(
					propConfig.getControlPropertyType());
			row[ModbusControlCsvColumn.REG_ADDR.getCode()] = (propConfig.getAddress() != null
					? propConfig.getAddress().toString()
					: null);
			row[ModbusControlCsvColumn.REG_TYPE.getCode()] = registerTypeValue(propConfig.getFunction());
			row[ModbusControlCsvColumn.DATA_TYPE.getCode()] = dataTypeValue(propConfig.getDataType());
			row[ModbusControlCsvColumn.DATA_LENGTH.getCode()] = dataLengthValue(propConfig);
			row[ModbusControlCsvColumn.MULTIPLIER.getCode()] = (propConfig.getUnitMultiplier() != null
