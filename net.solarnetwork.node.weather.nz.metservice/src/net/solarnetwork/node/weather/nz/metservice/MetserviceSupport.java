 ...
 *   <dt>baseUrl</dt>
 *   <dd>The base URL for queries to MetService. Defaults to {@link #DEFAULT_BASE_URL}.</dd>
 * </dl>
 * 
...
public abstract class MetserviceSupport<T extends Datum> extends XmlServiceSupport {
	...
	private static final Pattern SIMPLE_JSON_PATTERN 
		= Pattern.compile("\"?(\\w+)\"?\\s*:\\s*\"?([^\"]+)\"?\\s*[,}]");
	
	private Map<String, T> datumCache;
	 ...
	 * 
	 * <p>This method will call the {@link #parseSimpleJavaScriptObjectProperties(String)}
	 * method after reading the entire InputStream into memory. The InputStream is assumed
	 * to be text encoded in Unicode.</p>
	 * 
	 * @param in the InputStream to read
	 ...
	 * 
	 * <p>This method is not a true JavaScript or JSON parser. It uses simple regular
	 * expressions to extract JavaScript properties in the form</p>
	 * 
	 * <pre>"prop":"value"</pre>
	 * 
	 ...
	 * 
	 * <p>If the date cannot be parsed, <em>null</em> will be returned.</p>
	 * 
	 * @param key the attribute key to obtain from the {@code data} Map
	 * @param data the attributes
	 * @param dateFormat the date format to use to parse the date string
	 * @return the parsed {@link Date} instance, or <em>null</em> if an error occurs
	 ...
	 * 
	 * <p>If the Double cannot be parsed, <em>null</em> will be returned.</p>
	 * 
	 * @param key the attribute key to obtain from the {@code data} Map
	 * @param data the attributes
	 * @return the parsed {@link Double}, or <em>null</em> if an error occurs
	 * or the specified attribute {@code key} is not available
	 */
