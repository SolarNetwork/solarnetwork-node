/* ==================================================================
 * DatumControlCenterCsvConfigurer.java - 19/08/2025 10:51:33 am
 *
...
/**
 * Service that can configure {@link DatumControlCenterService} instances via
 * CSV resources.
 *
 * @author matt
 * @version 1.0
...
public class DatumControlCenterCsvConfigurer extends BasicIdentifiable
	...
	private static final Logger log = LoggerFactory.getLogger(DatumControlCenterCsvConfigurer.class);

	private final SettingsService settingsService;
	private final OptionalService<IdentityService> identityService;

	private String settingProviderId = DatumControlCenterService.SETTING_UID;
	 ...
	 */
	public DatumControlCenterCsvConfigurer(SettingsService settingsService,
	...
	public String getSettingUid() {
		return "net.solarnetwork.node.io.dnp3.controlcenter.csv";
	}

	@Override
	public String getDisplayName() {
		return "DNP3 Control Center CSV Configurer";
	}
	...
	public Iterable<Resource> currentSettingResources(String settingKey) {
		...
		return (byos.size() > 0 ? Collections
				.singleton(new ByteArrayResource(byos.toByteArray(), "DNP3 Control Center CSV") {
					...
					public String getFilename() {
						...
						return (nodeId != null
								? String.format("dnp3-control-center-config-solarnode-%d.csv", nodeId)
								: "dnp3-control-center-config.csv");
					}
	...
	public synchronized SettingsUpdates applySettingResources(String settingKey,
		...
		List<DatumControlCenterConfig> configs = new ArrayList<>(8);
		try {
			for ( Resource r : resources ) {
				try {
					this.lastImportException = null;
					this.lastImportMessages = null;
					List<DatumControlCenterConfig> rConfigs = parseControlCenterConfigs(r);
					if ( rConfigs != null ) {
			...
			if ( log.isInfoEnabled() ) {
				log.info("Parsed {} DNP3 Control Center configurations: [\n\t{}\n]", configs.size(),
	...
	private List<DatumControlCenterConfig> parseControlCenterConfigs(Resource resource)
			throws IOException {
		List<DatumControlCenterConfig> configs = new ArrayList<>(8);
	...
	private SettingsUpdates toSettingsUpdates(List<DatumControlCenterConfig> configs) {
		...
		for ( DatumControlCenterConfig config : configs ) {
