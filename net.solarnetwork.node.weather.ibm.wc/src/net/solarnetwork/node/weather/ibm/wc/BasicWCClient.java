...
public class BasicWCClient extends HttpClientSupport implements WCClient{
	...
	private String getURL(String requestURL, String location, String apiKey, String datumPeriod) {
		...
		return this.baseUrl + "/" + String.format(requestURL,datumPeriod)
				+ "?" + String.format(this.locationTemplate,location) + 
	...
	/**
	 * For the daily forecast, each part of the data (ie. max temp, min temp, narrative) is an array, with each entry corresponding with a day
	 * of the week.
	 * The more detailed features in "daypart" such as cloud cover are split into day and night, meaning two entries for each day.
	 */
	...
	//TODO valid time UTC
	@Override
	public Collection<GeneralDayDatum> readDailyForecast(String locationIdentifier, String apiKey, String datumPeriod) {
	...
	@Override
	public Collection<WCHourlyDatum> readHourlyForecast(String locationIdentifier, String apiKey, String datumPeriod) {
		if(locationIdentifier == null) {
			return null;
		}
