...
public class MasterDemo {
	...
	static void run(DNP3Manager manager) throws Exception {
		...
		Channel channel = manager.addTCPClient("client", LogMasks.NORMAL | LogMasks.APP_COMMS,
				ChannelRetry.getDefault(), "127.0.0.1", "0.0.0.0", 20000, new Slf4jChannelListener());
		...
		while ( true ) {
			...
			switch (line) {
				case ("quit"):
					return;
				case ("crob"):
					ControlRelayOutputBlock crob = new ControlRelayOutputBlock(ControlCode.LATCH_ON,
							(short) 1, 100, 100, CommandStatus.SUCCESS);
