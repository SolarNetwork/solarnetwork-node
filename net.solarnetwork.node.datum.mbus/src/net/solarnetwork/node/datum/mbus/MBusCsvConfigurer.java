/* ==================================================================
 * MBusCsvConfigurer.java - 30/09/2022 1:37:32 pm
 *
...
/**
 * Service that can configure {@link MBusDatumDataSource} instances via CSV
 * resources.
 *
 * @author matt
 * @version 1.2
 * @since 1.1
 */
public class MBusCsvConfigurer extends BasicIdentifiable
	...
	private static final Logger log = LoggerFactory.getLogger(MBusCsvConfigurer.class);

	private final SettingsService settingsService;
	private final OptionalService<IdentityService> identityService;

	private String settingProviderId = MBusDatumDataSource.SETTING_UID;
	 ...
	 */
	public MBusCsvConfigurer(SettingsService settingsService,
	...
	public String getSettingUid() {
		return "net.solarnetwork.node.datum.mbus.csv";
	}

	@Override
	public String getDisplayName() {
		return "M-Bus Datum Data Source CSV Configurer";
	}
	...
	public Iterable<Resource> currentSettingResources(String settingKey) {
		...
		try (CsvWriter writer = CsvWriter.builder().commentCharacter('!').build(byos)) {
			MBusDatumDataSourceConfigCsvWriter gen = new MBusDatumDataSourceConfigCsvWriter(writer);
		...
		return (byos.size() > 0
				? Collections.singleton(new ByteArrayResource(byos.toByteArray(), "M-Bus Device CSV") {
					...
					public String getFilename() {
						...
						return (nodeId != null
								? String.format("mbus-device-config-solarnode-%d.csv", nodeId)
								: "mbus-device-config.csv");
					}
	...
	public synchronized SettingsUpdates applySettingResources(String settingKey,
		...
		List<MBusDatumDataSourceConfig> configs = new ArrayList<>(8);
		try {
			for ( Resource r : resources ) {
				try {
					this.lastImportException = null;
					this.lastImportMessages = null;
					List<MBusDatumDataSourceConfig> rConfigs = parseMBusConfigs(r);
					if ( rConfigs != null ) {
			...
			if ( log.isInfoEnabled() ) {
				log.info("Parsed {} M-Bus Device configurations: [\n\t{}\n]", configs.size(),
	...
	private List<MBusDatumDataSourceConfig> parseMBusConfigs(Resource resource) throws IOException {
		List<MBusDatumDataSourceConfig> configs = new ArrayList<>(8);
		lastImportMessages = new ArrayList<>(8);
		MBusDatumDataSourceConfigCsvParser parser = new MBusDatumDataSourceConfigCsvParser(configs,
	...
	private SettingsUpdates toSettingsUpdates(List<MBusDatumDataSourceConfig> configs) {
		...
		for ( MBusDatumDataSourceConfig config : configs ) {
