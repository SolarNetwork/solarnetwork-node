/* ==================================================================
 * UnchangedPropertyDatumFiltersServiceTests.java - 3/07/2023 7:49:17 am
 * 
...
import net.solarnetwork.node.datum.filter.std.UnchangedPropertyDatumFilterService;
import net.solarnetwork.node.domain.datum.SimpleDatum;

/**
 * Test cases for the {@link UnchangedPropertyDatumFilterService} class.
 * 
 * @author matt
 * @version 1.0
 */
public class UnchangedPropertyDatumFiltersServiceTests {
	...
	private static final String PROP_3 = "wh";
	private static final String PROP_4 = "status";
	private static final int UNCHANGED_SECS = 10;
	private static final int UNCHANGED_PROP_SECS = 5;

	private UnchangedPropertyDatumFilterService xform;

	@Before
	public void setup() {
		xform = new UnchangedPropertyDatumFilterService();
		xform.setUid("Test Unchanged Property");
		xform.setSourceId("^S");
		xform.setUnchangedPublishMaxSeconds(UNCHANGED_SECS);
		// @formatter:off
		xform.setPropConfigs(new PropertyFilterConfig[] {
				new PropertyFilterConfig(PROP_1, null),
				new PropertyFilterConfig(PROP_3, UNCHANGED_PROP_SECS),
		});
		// @formatter:on
	}
	...
	public void unchanged() {
		...
		d.putSampleValue(DatumSamplesType.Instantaneous, PROP_2, 2);
		d.putSampleValue(DatumSamplesType.Accumulating, PROP_3, 3);
		d.putSampleValue(DatumSamplesType.Status, PROP_4, "4");
		SimpleDatum d2 = d.copyWithId(nodeId(d.getObjectId(), d.getSourceId(), start.plusSeconds(1)));
		SimpleDatum d3 = d.copyWithId(
				nodeId(d.getObjectId(), d.getSourceId(), start.plusSeconds(UNCHANGED_PROP_SECS)));
		SimpleDatum d4 = d
		...
		SimpleDatum d6 = d
				.copyWithId(nodeId(d.getObjectId(), d.getSourceId(), d5.getTimestamp().plusSeconds(1)));
		...
		DatumSamplesOperations result5 = xform.filter(d5, d5.getSamples(), null);
		DatumSamplesOperations result6 = xform.filter(d6, d6.getSamples(), null);

		// THEN
		assertThat("First datum not filtered", result1, is(sameInstance(d.getSamples())));

		DatumSamples expected2 = new DatumSamples(d);
		expected2.putInstantaneousSampleValue(PROP_1, null);
		expected2.putAccumulatingSampleValue(PROP_3, null);
		assertThat("Second datum within 1st time period properties removed",
				result2.differsFrom(expected2), is(false));

		DatumSamples expected3 = new DatumSamples(d);
		expected3.putInstantaneousSampleValue(PROP_1, null);
		assertThat("Third datum after 1st time period custom property not removed",
				result3.differsFrom(expected3), is(false));

		assertThat("Forth datum within 2nd time period properties removed",
				result4.differsFrom(expected2), is(false));
		assertThat("Fifth datum within 2nd time period not filtered", result5,
				is(sameInstance(d5.getSamples())));
		assertThat("Sixth datum after 2nd time period properties removed",
				result6.differsFrom(expected2), is(false));
	}
	...
	public void changed_propValueAdded() {
		...
		d2.putSampleValue(DatumSamplesType.Instantaneous, PROP_2, 2);
		d2.putSampleValue(DatumSamplesType.Accumulating, PROP_3, 1);
		...
		DatumSamples expected2 = new DatumSamples(d2);
		expected2.putInstantaneousSampleValue(PROP_1, null);
		assertThat("Second datum within 1st time period first property discarded but added property not",
				result2.differsFrom(expected2), is(false));

		DatumSamples expected3 = new DatumSamples(d3);
		expected3.putInstantaneousSampleValue(PROP_1, null);
		expected3.putAccumulatingSampleValue(PROP_3, null);
		assertThat("Third datum within 2nd time period with same property value filtered",
				result3.differsFrom(expected3), is(false));
	}
	...
	public void emptyToNull() {
		...
		// THEN
		assertThat("First datum not discarded", result1, is(sameInstance(d.getSamples())));
		assertThat("Second datum within 1st time period discarded", result2, is(nullValue()));
	}

	@Test
	public void emptyPreserved() {
		// GIVEN
		xform.setPreserveEmptyDatum(true);
		Instant start = Instant.now().truncatedTo(ChronoUnit.SECONDS);
		SimpleDatum d = createTestDatum(start, SOURCE_ID_1, PROP_1, 1);
		SimpleDatum d2 = createTestDatum(start.plusSeconds(1), SOURCE_ID_1, PROP_1, 1);
		...
		// THEN
		assertThat("First datum not discarded", result1, is(sameInstance(d.getSamples())));
		assertThat("Second datum within 1st time period preserved (empty)", result2.isEmpty(), is(true));
	}
